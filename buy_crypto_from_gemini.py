...
def get_fear_and_greed_index():
    ...
    return dict(
        value=fear_and_greed_value, classification=fear_and_greed_classification
    )
...
def place_buy_order(options):
    ...
    execution_price = str(
        round(spot_price * options["orderFillFactor"], quote_increment)
    )
    # get tick size
    tick_size = get_tick_size(options)
    """
    if you want to consider the fear and greed data and the fear is less than
    your specified fear floor or greater than your specified greed ceiling adjust the amount you're buying
    """
    ...
    amount = str(
        round((options["amount"] * FEE_FACTOR) / float(execution_price), tick_size)
    )
    # execute maker buy with the appropriate symbol (options["currency"]), amount, and calculated price
    buy_order = trader.new_order(
        options["currency"], amount, execution_price, "buy", ["maker-or-cancel"]
    )
    return buy_order
